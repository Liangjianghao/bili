...
#   `videos` int(11) DEFAULT NULL COMMENT '视频数量',
#   `tid` int(11) DEFAULT NULL COMMENT '类别id',
#   `tname` varchar(255) DEFAULT NULL COMMENT '类别',
#   `copyright` int(11) DEFAULT NULL COMMENT '自制',
#   `duration` int(11) DEFAULT NULL COMMENT '时长',
#   PRIMARY KEY (`Id`)
# ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='bili总表';

class BiliallPipeline(object):
    def __init__(self):
    ...
    def process_item(self, item, spider):
        ...
        if len(self.item_list) >=100000:
            self.insert_item_list(spider)

    # videoTitle, videoUrl, videoCover, userName, uploadTime, playNum, danmuNum, contenNum, saveNum, coinNum, likeNum, dislikeNum, shareNum, danmuID,videos,tid,tname,copyright,duration
    def insert_item_list(self,spider):
        try:
            insertStr = 'insert into biliAll (videoTitle,videoUrl,videoCover,userName,uploadTime,playNum,danmuNum,contenNum,saveNum,coinNum,likeNum,dislikeNum,shareNum,danmuID,videos,tid,tname,copyright,duration)  values'
            for index,item in enumerate(self.item_list):
                print('%s--%s'%(index+1,item['videoTitle']))
                itemtr="('%s','%s', '%s','%s','%s','%s','%s', '%s','%s','%s','%s','%s', '%s','%s','%s','%s','%s', '%s','%s')"% \
                       (item['videoTitle'],item['videoUrl'],item['videoCover'],item['userName'],item['uploadTime'],item['playNum'],
                        item['danmuNum'],item['contenNum'],item['saveNum'],item['coinNum'],item['likeNum'],item['dislikeNum'],item['shareNum'],item['danmuID'],item['videos'],item['tid'],item['tname'],item['copyright'],item['duration'])
